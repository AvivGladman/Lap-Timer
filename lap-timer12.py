...
def create_brocade(self):
    # Brocade: Gold streaks with long trailing effect, like weeping willow
    colors = [
        (255, 215, 0),    # Pure gold
        (255, 223, 0),    # Light gold
        (255, 193, 37),   # Rich gold
        (255, 140, 0),    # Orange-gold
        (255, 255, 224),  # Very light gold/cream
    ]
    ...
    for _ in range(num_particles):
        ...
        # Vary the speed more dramatically for realistic spread
        speed = random.uniform(2, 8)
        
        # Add slight upward bias to simulate gravity effects
        vx = math.cos(angle) * speed
        vy = math.sin(angle) * speed - random.uniform(0.5, 1.5)
        
        # Much longer life for trailing effect
        life = random.randint(80, 120)
        
        # Weight color selection toward gold tones
        color_weights = [0.4, 0.3, 0.15, 0.1, 0.05]
        color = random.choices(colors, weights=color_weights)[0]
        
        # Create particle with longer trail and larger size
        particle = Particle(
            self.x, self.y, vx, vy, color, life, 
            trail_length=random.randint(15, 25),  # Much longer trails
            size=random.randint(2, 4)  # Varied sizes
        )
        
        # Add some particles that fade more gradually
        particle.fade_rate = random.uniform(0.015, 0.03)
        
        self.particles.append(particle)
    
    # Add some delayed secondary particles for crackling effect
    num_secondary = random.randint(10, 20)
    for _ in range(num_secondary):
        ...
        # Slightly dimmer colors for secondary particles
        secondary_colors = [(200, 170, 0), (220, 190, 20), (180, 150, 0)]
        color = random.choice(secondary_colors)
        ...
        # Delay the secondary particles slightly
        particle.delay = random.randint(10, 30)
