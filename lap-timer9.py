...
font_huge = pygame.font.SysFont(None, min(screen_width//6, screen_height//6))
font_large = pygame.font.SysFont(None, min(screen_width//10, screen_height//10))
font_medium = pygame.font.SysFont(None, min(screen_width//15, screen_height//15))
font_small = pygame.font.SysFont(None, min(screen_width//25, screen_height//25))
...
# === UI Buttons ===
# Top control buttons
top_button_width = screen_width // 6
top_button_height = screen_height // 12
top_button_y = 30

quit_button_rect = pygame.Rect(screen_width - top_button_width - 30, top_button_y, top_button_width, top_button_height)
reset_button_rect = pygame.Rect(30, top_button_y, top_button_width, top_button_height)

# Main control buttons (bottom area)
main_button_width = screen_width // 5
main_button_height = screen_height // 10
main_button_y = screen_height - main_button_height - 40
main_button_spacing = 40

start_button_rect = pygame.Rect(60, main_button_y, main_button_width, main_button_height)
...
lap_button_width = screen_width // 6
lap_button_height = screen_height // 7
lap_buttons_y = screen_height // 2 - lap_button_height // 2
lap_buttons_spacing = 40
...
length_button_width = screen_width // 3
length_button_height = screen_height // 7
length_buttons_y = screen_height // 2 - length_button_height // 2
length_buttons_spacing = 50
...
def draw_display(elapsed_time, laps_left, finished):
    ...
    # Calculate safe area below top buttons
    safe_area_top = top_button_y + top_button_height + 50

    if input_stage == "laps":
        # Display lap selection buttons
        prompt_surface = font_large.render("Select number of laps:", True, (255, 255, 255))
        prompt_rect = prompt_surface.get_rect(center=(screen_width // 2, safe_area_top + 80))
        ...
        if length_input_mode == "buttons":
            ...
            input_rect = input_surface.get_rect(center=(screen_width // 2, safe_area_top + 200))
            screen.blit(input_surface, input_rect)
            
            instruction_surface = font_medium.render("Press Enter to confirm", True, (200, 200, 200))
            instruction_rect = instruction_surface.get_rect(center=(screen_width // 2, safe_area_top + 320))
            screen.blit(instruction_surface, instruction_rect)
        
    else:
        # Timer display area
        timer_y = safe_area_top + 60
        ...
        # Status display area - position above control buttons
        status_start_y = timer_y + 150
        
        if finished:
            laps_surface = font_large.render(f"Total Laps: {total_laps}", True, (255, 200, 200))
            laps_rect = laps_surface.get_rect(center=(screen_width // 2, status_start_y))
            screen.blit(laps_surface, laps_rect)
            
            distance = total_laps * lap_length_m * 2
            distance_surface = font_large.render(f"Distance: {distance:.2f} m", True, (200, 255, 200))
            distance_rect = distance_surface.get_rect(center=(screen_width // 2, status_start_y + 80))
            screen.blit(distance_surface, distance_rect)
            
            congrats_surface = font_large.render("Congratulations!!!", True, (100, 255, 100))
            congrats_rect = congrats_surface.get_rect(center=(screen_width // 2, status_start_y + 160))
            ...
            laps_remaining_rect = laps_remaining_surface.get_rect(center=(screen_width // 2, status_start_y))
            ...
            completed_rect = completed_surface.get_rect(center=(screen_width // 2, status_start_y + 80))
